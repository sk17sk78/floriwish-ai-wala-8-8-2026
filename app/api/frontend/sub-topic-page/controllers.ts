...
// controllers
export const getSubTopicMeta = async ({
  categorySlug,
  topicSlug,
  subTopicSlug
}: {
...
export const getSubTopicPageSlugs = async (): Promise<
  SubTopicDocument[] | null
> => {
  try {
    await connectDB();
...
export const getSubTopicPageDetailsI = async ({
  categorySlug,
  topicSlug,
  subTopicSlug
}: {
  ...
  try {
    ...
    const document = await SubTopics.findOne({
      isActive: true,
      topic: topic._id,
      slug: subTopicSlug
    })
...
export const getSubTopicPageDetailsII = async ({
  categorySlug,
  topicSlug,
  subTopicSlug
}: {
  ...
  try {
    ...
    const documentObj = document.toObject() as SubTopicDocument;
...
export const getSubTopicPageContents = async ({
  categorySlug,
  topicSlug,
  subTopicSlug
}: {
