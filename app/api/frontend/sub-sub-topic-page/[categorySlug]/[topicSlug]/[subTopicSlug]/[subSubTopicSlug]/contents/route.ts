...
// controllers
import { getSubSubTopicPageContents } from "../../../../../controllers";

// constants
import { SUB_SUB_TOPIC_PAGE_CONTENTS_CACHE_KEY } from "@/common/constants/cacheKeys";
...
export const GET = async (
  ...
  try {
    const cachedDocuments = await getFromRedis<ContentDocument[]>({
      key: `${SUB_SUB_TOPIC_PAGE_CONTENTS_CACHE_KEY}_${categorySlug}_${topicSlug}_${subTopicSlug}_${subSubTopicSlug}`
    });

    if (!cachedDocuments) {
      const documents = await getSubSubTopicPageContents({
        categorySlug,
        topicSlug,
        subTopicSlug,
        subSubTopicSlug
