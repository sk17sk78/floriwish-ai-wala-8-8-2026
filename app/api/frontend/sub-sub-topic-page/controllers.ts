...
const { ContentCategories, Topics, SubTopics, SubSubTopics } = models;
...
// controllers
export const getSubSubTopicMeta = async ({
  categorySlug,
  topicSlug,
  subTopicSlug,
  subSubTopicSlug,
...
}): Promise<SubSubTopicDocument | null> => {
  try {
    await connectDB();
...
export const getSubSubTopicPageSlugs = async (): Promise<
  SubSubTopicDocument[] | null
> => {
  try {
    await connectDB();

    const documents = await SubSubTopics.find({
      isActive: true
    })
      .select(["slug"])
      .populate([
...
export const getSubSubTopicPageDetailsI = async ({
  categorySlug,
  topicSlug,
  subTopicSlug,
  subSubTopicSlug
  ...
  try {
    ...
    const document = await SubSubTopics.findOne({
      isActive: true,
      topic: topic._id,
      subTopic: subtopic._id,
      slug: subSubTopicSlug
...
export const getSubSubTopicPageDetailsII = async ({
  categorySlug,
  topicSlug,
  subTopicSlug,
  subSubTopicSlug
  ...
  try {
    ...
    const documentObj = document.toObject() as SubSubTopicDocument;
...
export const getSubSubTopicPageContents = async ({
  categorySlug,
  topicSlug,
  subTopicSlug,
  subSubTopicSlug
