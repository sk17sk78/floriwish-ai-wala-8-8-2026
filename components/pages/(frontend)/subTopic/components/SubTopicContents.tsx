...
// requests
import { fetchSubTopicContents } from "@/request/page/subTopicContents";
...
function SubTopicContents({
  ...
  useEffect(() => {
    if (canLoadMore && !isLoaded && (loadMore || changedSortBy)) {
      fetchSubTopicContents({ categorySlug, topicSlug, subTopicSlug })
        .then(({ data }) => {
...
export default memo(SubTopicContents);
