...
import { createSubSubTopicAction, selectSubSubTopic } from "@/store/features/pages/subSubTopicSlice";

export default function TableFormFields({
  initialDocument
}: {
  initialDocument?: SubSubSubTopicDocument;
}) {
  // hooks
  ...
  const { documents: subSubTopics, options: subSubTopicOptions } = useSelector((state) =>
  ...
  const [subTopic, setSubTopic] = useState<string>(
    (initialDocument?.subTopic as string) || ""
  );
  ...
  const validSubSubTopicOptions = subSubTopicOptions.filter(
    ({ value }) =>
      subSubTopics.find(({ _id }) => String(_id) === value)?.category === category
      && subSubTopics.find(({ _id }) => String(_id) === value)?.topic === topic
      && subSubTopics.find(({ _id }) => String(_id) === value)?.subTopic === subTopic
  );
  ...
  useEffect(() => {
    if (subSubTopicStatus === "idle") {
      dispatch(createSubSubTopicAction.fetchDocuments());
    }
  ...
  useEffect(() => {
    if (initialDocument) {
      ...
      setSubTopic(initialDocument?.subTopic as string);
  ...
  // Only auto-generate slug for new subSubSubTopic creation if slug not manually set
  useEffect(() => {
  ...
  return (
    <section className="grid grid-cols-1 gap-4 w-[70vw] max-h-[calc(100dvh_-_200px)] px-2 overflow-y-scroll scrollbar-hide pb-20">
      ...
      <Input
        ...
        nullOption
        customInitialValuePlaceholderLabel={
          topic ? "Select Category 3" : "Category 2 Not Selected"
        }
      ...
      <Input
        ...
        isRequired
        isDisabled={!topic || !subTopic}
        nullOption
        customInitialValuePlaceholderLabel={
          topic ? subTopic ? "Select Category 4" : "Category 3 Not Selected" : "Category 2 Not Selected"
        }
        options={validSubSubTopicOptions}
        defaultValue={initialDocument?.subSubTopic as string}
        errorCheck={false}
        validCheck={false}
      />
      <Input
