// components
import Link from "next/link";
import NextImage from "@/components/custom/NextImage";
import { Star } from "lucide-react";
import { alwaysDecimal } from "@/components/pages/(frontend)/Content/components/Details/helpers/alwaysDecimal";
import { OPTIMIZE_IMAGE } from "@/config/image";
import { SearchContentsType } from "../SearchContentUI";

export default function SearchResultContents({
  contents,
  indices,
  collapse
}: {
  contents: SearchContentsType[];
  indices: number[];
  collapse: () => void;
}) {
  // hooks
  // const { contentResults, onShowSearchPage } = useSearch();

  return indices.length > 0 ? (
    <section className="flex flex-col relative">
      <span className="font-medium text-lg text-charcoal-3 max-sm:pt-2 pb-2">
        Products and Services
      </span>
      <section className="relative flex flex-col gap-3 max-h-[75dvh] sm:max-h-[40dvh] overflow-y-scroll scrollbar-hide">
        {indices.slice(0, 300).map((i, index) => (
          <Link
            key={index}
            className="grid grid-cols-[53px_1fr] gap-2 sm:gap-3 items-start rounded-xl py-2 group hover:text-sienna"
            href={contents[i].slug}
            prefetch
            onClick={collapse}
          >
            <div className="relative overflow-hidden aspect-square rounded-lg">
              <NextImage
                className="w-full h-full object-cover object-center"
                src={contents[i].image}
                alt={"Content Image"}
                width={53}
                height={53}
                draggable={false}
              />
            </div>
            <section className="flex flex-col justify-start">
              <span className="pb-0.5 line-clamp-1 transition-all duration-300">
                {contents[i].name}
              </span>
              <div className="flex items-center justify-between">
                <span className="font-medium transition-all duration-300 text-[17px]">{`₹${contents[i]}`}</span>
                {contents[i].rating > 0 ? (
                  <section className="flex items-center gap-1 bg-green-600 text-white px-2 py-0.5 rounded-md">
                    <Star
                      fill="#fff"
                      width={12}
                      height={12}
                    />
                    <span className="text-xs font-semibold">
                      {alwaysDecimal(contents[i].rating)}
                    </span>
                  </section>
                ) : (
                  <></>
                )}
              </div>
            </section>
          </Link>
        ))}
      </section>
      {/* {contentResults.length > 10 && (
        <span
          className="font-medium w-full text-center cursor-pointer border-t border-charcoal-3/50 pt-2.5 pb-3 sticky bottom-0 bg-ivory-1"
          onClick={() => {
            onShowSearchPage();
          }}
        >
          View All
        </span>
      )} */}
    </section>
  ) : (
    <></>
  );
}
