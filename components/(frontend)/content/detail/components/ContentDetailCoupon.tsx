...
export default function ContentDetailCoupon({
  price: contentPrice,
  coupon
}: {
  ...
  const isApplicable = coupon.minimumOrderAmount <= price;
    ...
    coupon.type === "discount" && coupon.discount
      ? coupon.discount.type === "percentage" && coupon.discount?.percentage
        ? Math.min(
            ...
            Math.ceil((price * coupon.discount.percentage) / 100)
          )
        : coupon.discount.limit
      : 0;
          ...
          {coupon.type === "discount" && (
            ...
            >
              {isApplicable
                ...
                : `Add items worth ${INRSymbol}${coupon.minimumOrderAmount - price} more to unlock`}
            </span>
          )}
        </div>
      </div>
