...
export default function FooterBottomRight() {
  return (
    <section className="max-sm:col-start-1 max-sm:row-start-3 flex max-sm:flex-col items-center justify-center sm:justify-end max-sm:py-5 gap-1.5 sm:gap-3.5 text-charcoal-3 sm:text-charcoal-3 text-sm">
...
export const Social = ({
  ...
  return (
    <Link
      href={link}
      aria-label={label}
      className={`text-charcoal-2 sm:text-charcoal-2 min-w-fit px-1 transition-all duration-300 hover:text-sienna hover:brightness-75 ${className || ""}`}
    >
      {children}
    </Link>
  );
